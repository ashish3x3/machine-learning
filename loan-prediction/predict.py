...
def compute_false_negatives(target, predictions):
    ...
    print 'df[(df["target"] == 1) & (df["predictions"] == 0)].shape[0] ',df[(df["target"] == 1) & (df["predictions"] == 0)].shape[0]
    print 'df[(df["target"] == 1)].shape[0] + 1 ',df[(df["target"] == 1)].shape, df[(df["target"] == 1)].shape[0] + 1
    print 'div ',
...
if __name__ == "__main__":
    ...
    print 'predictions ',predictions,train[settings.TARGET]
    ...
    print("Accuracy Score: {0:.3f}".format(error))
    print("False Negatives: {0:.3f}".format(fn))
    print("False Positives: {0:.3f}".format(fp))
